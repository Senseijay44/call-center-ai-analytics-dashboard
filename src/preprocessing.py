...
def get_column_map(df: pd.DataFrame) -> dict:
    return {
        "transcript": find_best_column(df, ["transcript", "text", "call_transcript"]),
        "sentiment": find_best_column(df, ["sentiment", "emotion", "sentiment_label"]),
        "call_type": find_best_column(
            df,
            [
                "call_type",
                "type",
        ...
        "product": find_best_column(
            df,
            ["product_code", "product", "product_id", "product_number", "item", "item_code", "sku", "product_name"],
        ),
        "caller": find_best_column(df, ["caller_name", "customer_name", "name"]),
        "order": find_best_column(df, ["order_number", "order_id"]),
        "timestamp": find_best_column(df, ["timestamp", "call_time", "datetime", "created_at"]),
        "confidence": find_best_column(df, ["confidence", "confidence_score", "intent_confidence", "model_confidence"]),
    }
